...
function UasrRepository(dbContext) {
    ...
    function findUasr(req, res, next) {
        ...
        if (req.params.uasId) {
            ...
            var query = "select uasname from dbo.approved where uasname = @uasname"
    ...
    function getUasr(req, res) {

        var query = "select uasname from dbo.approved order by uasname"
    ...
    function postUasr(req, res) {
        ...
        var query = "insert into dbo.approved values(@uasname, @baselinecheck, @physicalarea, @uasdragcoef, @maxthrust, @propellerdiameter, @propellerweight, " +
